...
<%-- taglib 디렉티브를 사용하여 JSTL의 Core 태그 라이브러리를 JSP 문서에 포함해야만 
    Core 태그 라이브러리를의 커스텀 태그를 제공받아 사용 가능 --%>
...
<body>
	...
	<%-- var 속성 : 스코프 객체에 저장될 속성값을 구분하기 위한 속성명을 속성값으로 설정 --%>
	...
	<%-- set 태그의 value 태그 속성값으로 EL를 이용하여 값을 제공받아 속성값으로 저장 가능 --%>
	...
	메소드를 호출하여 반환받은 문자열을 제공받아 출력 처리 --%>
	<p>now = ${now }</p>
